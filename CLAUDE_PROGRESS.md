...
## Current task — IGNYT Coach sync client — COMPLETE, built, committed

Request: write the sync client so a coach's assigned workouts and meal plans arrive in this
app, and the user's activity goes back to the coach's dashboard.

Branch: `feature/coach-sync` (from `main`).

**What the coach platform is.** IGNYT Coach is a SEPARATE application in its own repository
(`../IGNYT-Coach`), with its own database and deployment. Nothing of it lives here. The only
coupling is the HTTP contract this file implements.

### Files
- **NEW** `www/js/trainer-sync.js` — the whole client. Global `IgnytTrainerSync`.
- `www/index.html` — one `<script>` tag, loaded last (needs app.js globals + IgnytAuth).
- `www/sw.js` — asset added to the precache list; `CACHE` bumped v80 → v81.
- `www/app.js` — 1 line: dispatches `ignyt:workout-finished` after a real commit, wrapped in
  try/catch so a listener can never fail a workout that is already saved.

Total change to existing files: **10 inserted lines.** Nothing removed, nothing renamed.

### Naming
`IgnytTrainerSync`, not `IgnytCoachSync` — `js/coach/*` (`IgnytCoachEngine`) is this app's
offline AI programme generator. Different thing, and a collision there would be nasty.

### Storage
Reads `hx_workout_log`, `hx_food_log`, `hx_bodylog`, `hx_water_log` — never writes them.
Writes new keys `hx_trainer_link|cursor|meal_plans|targets|schedule`, plus `hx_routines`.

`hx_routines` is the ONLY existing key written, and it is the point of the feature. The merge
is id-scoped: coach records are prefixed `coach:`, `nextId()` only issues Date.now() numbers,
so they cannot collide. User routines are never read, reordered or rewritten. Unlink removes
the `coach:` slice and nothing else.

Coach calorie/macro targets are deliberately NOT written into `hx_nutrition` — that holds the
user's own macro percentages and overwriting it would replace a choice they made.

### Field mappings that were wrong until the real shapes were read
- `durationMin` (minutes) → server `duration` (**seconds**). ×60.
- `title` → server `name`.
- Sets: only `done: true` are sent. A live session carries planned-but-unperformed sets;
  pushing them would inflate volume, compliance and PRs with work that did not happen.
- `servingUnit` → `unit`; `grams` → `quantity`.
- `bodyfat` → `bodyFat`; `leftCalf` → `calf`. Values arrive from DOM inputs as strings.
- Water is one row PER ADDITION; the server keeps one total per day. Summed client-side, or
  the last sip of the day would overwrite the total.

### Bugs found and fixed in the coach API while testing this
1. `touch_cursor` incremented `revision` before the column default applied → **every first
   push from a newly linked client was a 500.** No unit test caught it; only the real client
   did. Regression test added (`api/tests/test_sync_endpoints.py`).
2. The link endpoint's rate limiter ignored `RATE_LIMIT_ENABLED`, so it could not be turned
   off — untestable, and the setting was a lie.
3. `require_client` and `/sync/link` had no dev-auth path, so the entire sync surface could
   not be exercised without a Firebase project. Both now honour `AUTH_MODE=insecure-uid`
   (refused in production by `assert_ready()`).

### Verification
- End-to-end harness loaded the real module in a browser-shaped VM against a live API:
  **40/40 checks pass** — mapping, link, push, pull, idempotency on repeat sync, user
  routines surviving a merge, unlink removing only coach data.
- Coach API suite: **81 passed** (12 new sync-endpoint tests).
- `npx cap sync android` — OK, module copied into `android/app/src/main/assets/public/js/`.
- `.\gradlew.bat clean assembleDebug` — **BUILD SUCCESSFUL** (1m 21s).
- APK: `android/app/build/outputs/apk/debug/app-debug.apk`.

### Not done
- **No UI.** There is no screen to enter a link code, and no coach section in Profile. The
  module is complete and callable (`IgnytTrainerSync.link(code)`), but until a screen exists
  a user cannot reach it. This is the next task.
- **No FCM handler.** `data.type = plan.assigned` should trigger `sync()`; the app has an
  `IgnytNotify` plugin but nothing routes a data-message to JS.
- Progress photos are not synced (no upload path on either side).
- Sync triggers are foreground / online / workout-finished, with a 5-minute floor. Not
  verified on a physical device over mobile data.

---

## Previous task — 8-item backlog, ALL COMPLETE, one branch per item, all pushed
